...
my $i = 1;
my @arr;
my $package = "CNN35XX-NFBE-Linux-Driver-KVM-XEN-PF-SDK-2.0-122.tgz";

foreach my $box (@boxes){
my $cfm2util="/root/cnn35xx-nfbe-kvm-xen-pf/software/bindist/Cfm2Util";
my $au="$cfm2util loginHSM -u AU -s app_user -p user123";
#my $code = system("ping -c 2 $box");
#if ($code eq 0){
print "box: $box\t";
        #prepscript($i);
        #my $log="log$i.sh";
        #system("ping $box");
        #system("sshpass -p a scp $package $box:");
        #system("sshpass -p a ssh -o \"StrictHostKeyChecking no\" $box \"poweroff\"");
